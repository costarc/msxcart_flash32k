...
; -------------------
; Cart search
; Searches cartrife in ROM
; ------------------
checkFlash:
	ld		de, strProcuraCart
...
.loopck:
	...
	jr		c, .loopck					; Not found in this slot, continue with next one
...
.loopwr:
	...
	ld		a, b
	or		c
	jp		nz, .loopwr					; All 16 KB programmed? Return for making next block.
.erro:
	push	af
	xor		a
...
;	ld		(erasedelay), a				; delay de 256 ciclos
	...
	ld		a, (hl)
	ld		(togglebit), a
.loopera:									; testa fim do erase por Toggle Bit
	ld		a, (togglebit)
	cp		(hl)
	ld		a, (hl)
	ld		(togglebit), a
...
.idp:
	...
	jr		z, .okid						; beleza, identificamos a flash
	ld		e, 6
...
strTitulo:
	db		"Cartridge search utility",13,10
	db		"(c) 2017 by Ronivon Costa"

strCrLf:
	db		13, 10, '$'

strProcuraCart:
	db		"Searching Cartridge in system ...",13,10
	db		'$'

strNaoAchado:
	db		"Cartridge not Found!!", 13, 10
	db		'$'
