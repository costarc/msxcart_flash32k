...
.loopk:
	...
	jr		c, .loopk					; Not found in this slot, continue with next one
...
.loopg:
	...
	ld		a, b
	or		c
	jp		nz, .loopg					; All 16 KB programmed? Return for making next block.
.erro:
	push	af
	xor		a
	out		(IOFW), a					; Desliga modo gravacao da Flash
...
;	ld		(erasedelay), a				; delay de 256 ciclos
	...
	ld		a, (hl)
	ld		(togglebit), a
.looph:									; testa fim do erase por Toggle Bit
	ld		a, (togglebit)
	cp		(hl)
	ld		a, (hl)
	ld		(togglebit), a
